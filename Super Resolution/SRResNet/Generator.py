...
class Generator(nn.Module):
    ...
    def __init__(self, base_channels=64, n_ps_blocks=2, n_res_blocks=16):
        ...
        for _ in range(n_ps_blocks):
            ps_blocks += [
                nn.Conv2d(base_channels, 4 * base_channels, kernel_size=3, padding=1),  # Only increase the channels not the pixel
                nn.PixelShuffle(2),     # Increase the pixel dimension
                nn.PReLU(),
            ]
